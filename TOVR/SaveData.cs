...
namespace TOVR
{
	class SaveData
	{
		...
		public bool Open(String filename)
		{
			...
			{
				if (buffer[i + 0x228] != header[i])
				{
					check = false;
					break;
				}
			}
			if (check) mFormat = new FileFormatSwitch();
			...
			if (check) mFormat = new FileFormatRaw();
			...
			if (mBuffer != null) mFileName = filename;
			Backup();
			return true;
		}
		...
		public bool Save()
		{
			if (mFileName == null || mBuffer == null || mFormat == null) return false;
